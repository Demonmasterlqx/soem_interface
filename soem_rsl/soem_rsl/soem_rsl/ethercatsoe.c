...
int ecx_readIDNmap(ecx_contextt *context, uint16 slave, int *Osize, int *Isize)
{
   ...
   ec_soem_rslappingt     soem_rslapping;
   ...
   *Osize = 0;
   ...
   {
      psize = sizeof(soem_rslapping);
      /* read output mapping via SoE */
      wkc = ecx_SoEread(context, slave, driveNr, EC_SOE_VALUE_B, EC_IDN_MDTCONFIG, &psize, &soem_rslapping, EC_TIMEOUTRXM);
      if ((wkc > 0) && (psize >= 4) && ((entries = etohs(soem_rslapping.currentlength) / 2) > 0) && (entries <= EC_SOE_MAXMAPPING))
      {
         ...
         *Osize += 16;
         for (itemcount = 0 ; itemcount < entries ; itemcount++)
         {
            ...
            wkc = ecx_SoEread(context, slave, driveNr, EC_SOE_ATTRIBUTE_B, soem_rslapping.idn[itemcount], &psize, &SoEattribute, EC_TIMEOUTRXM);
      ...
      wkc = ecx_SoEread(context, slave, driveNr, EC_SOE_VALUE_B, EC_IDN_ATCONFIG, &psize, &soem_rslapping, EC_TIMEOUTRXM);
      if ((wkc > 0) && (psize >= 4) && ((entries = etohs(soem_rslapping.currentlength) / 2) > 0) && (entries <= EC_SOE_MAXMAPPING))
      {
